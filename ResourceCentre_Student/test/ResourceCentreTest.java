...
<<<<<<< HEAD
		...
		// Item list is not null, so that can add a new item
				...
				//The item just added is as same as the first item of the list
				assertSame("Test that Chromebookr is added same as 1st item of the list?", cb1, chromebookList.get(0));
...
=======
		...
		// Item list is not null, so that can add a new item
				...
				assertEquals("Test that Chromebook arraylist size is 3?", 3, chromebookList.size());
				assertSame("Test that Chromebook is added same as 3rd item of the list?", cb3, chromebookList.get(2));
>>>>>>> branch 'master' of https://github.com/21039443-Chua-Liwen/ResourceCentre_Student.git
	}
...
<<<<<<< HEAD
		//boundary
				...
				assertFalse("Test if available hromebook CB0011 is returned -false?", isReturned);		
				//normal
				ResourceCentre.addChromebook(chromebookList, cb2);
				cb2.setIsAvailable(false);
